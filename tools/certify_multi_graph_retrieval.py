...
from syntavra_runtime.multi_graph_retrieval import GRAPH_KINDS, GraphEdge, GraphNode, MultiGraphRetrieval
...
def _runtime_smoke() -> dict[str, Any]:
        ...
        engine.add_layer(
            ...
            [
                GraphNode(
                    ...
                    metadata={"freshness": "current", "disposition": "allow"} if graph_kind == "security" else {"freshness": "current"},
                )
            ],
    ...
    _require(result["candidate_count"] == 1, "all graph views did not fuse to one context identity")
    candidate = result["candidates"][0]
    _require(candidate["graph_kinds"] == sorted(GRAPH_KINDS), "eight-graph fusion coverage drift")
    _require(candidate["item_id"] == "certifier-shared-context", "item identity fusion drift")
    _require(len(result["receipt"]["receipt_hash"]) == 64, "retrieval receipt hash drift")

    deny = MultiGraphRetrieval()
    deny.add_layer(
        "code",
        "code",
        [GraphNode("code", "code", "sensitive migration", namespace_uri=shared_uri, trust_level="verified")],
    )
    deny.add_layer(
        "security",
        "security",
    ...
    _require(denied["candidate_count"] == 0, "security deny did not fail closed")
    ...
    propagated.add_layer(
        ...
        "causal",
        [
            GraphNode("causal", "symptom", "request timeout", trust_level="verified"),
            GraphNode("causal", "cause", "pool exhaustion", trust_level="verified"),
        ],
        [GraphEdge("symptom", "cause", "caused-by", confidence=0.95, evidence_refs=("trace:fixture",))],
    )
    propagated_result = propagated.retrieve("request timeout", max_hops=2)
    _require(
        any(row["identity"] == "node:causal:cause" for row in propagated_result["candidates"]),
        "bounded graph propagation drift",
    )

    status = engine.status()
    _require(status["persistent_store"] is False, "Multi-Graph invented a persistent store")
    _require(status["payload_authority"] is False, "Multi-Graph claimed payload authority")
    return {
...
def _validate_workflow(workflow: str) -> None:
    ...
    _require(external == EXPECTED_ACTION_REFS, f"Multi-Graph external action set drift: {sorted(external)}")
    for ref in external:
...
def _validate_enforcement(repo: Path) -> dict[str, str]:
    for relative in (WORKFLOW_RELATIVE, RELEASE_GATE_RELATIVE, TEST_RELATIVE):
        _require((repo / relative).is_file(), f"missing Multi-Graph enforcement surface: {relative.as_posix()}")
    workflow = (repo / WORKFLOW_RELATIVE).read_text(encoding="utf-8")
    _require(
        "group: multi-graph-retrieval-${{ github.event.pull_request.number || github.ref }}" in workflow,
        "Multi-Graph concurrency is not PR/ref scoped",
    )
    _require("tests.runtime.test_multi_graph_retrieval" in workflow, "Multi-Graph workflow lost regression suite")
    _require("tools/certify_multi_graph_retrieval.py" in workflow, "Multi-Graph workflow lost certifier")
...
def certify(repo: Path) -> dict[str, Any]:
    repo = repo.resolve()
    _require(repo == ROOT, f"Multi-Graph certifier must run against its own checkout: {repo} != {ROOT}")
    contract = _read_json(repo / CONTRACT_RELATIVE)
    _require(contract.get("schema_version") == 1, "Multi-Graph schema drift")
    _require(contract.get("family") == "multi-graph-retrieval", "Multi-Graph family drift")
    _require(contract.get("phase") == "python-first", "Multi-Graph phase drift")
    _require(contract.get("claim") == "MULTI_GRAPH_RETRIEVAL_V1", "Multi-Graph claim drift")
    _require(contract.get("strict") is True, "Multi-Graph must remain strict")
    _require(contract.get("runtime") == RUNTIME_RELATIVE.as_posix(), "Multi-Graph runtime path drift")
    _require(set(contract.get("graph_kinds") or ()) == GRAPH_KINDS, "Multi-Graph kind coverage drift")
    ...
    for key in (
        ...
        _require(ownership.get(key) is True, f"Multi-Graph ownership policy disabled: {key}")
    _require(ownership.get("public_cli_route_added") is False, "Multi-Graph invented a public CLI route")
    ...
    for key in (
        ...
        _require(retrieval.get(key) is True, f"Multi-Graph retrieval policy disabled: {key}")
    _require(retrieval.get("bounded_limit") == 100, "Multi-Graph limit boundary drift")
    _require(retrieval.get("bounded_hops") == 4, "Multi-Graph hop boundary drift")
    ...
    for key in (
        ...
        _require(receipt.get(key) is True, f"Multi-Graph receipt policy disabled: {key}")

    completeness = certify_completeness(repo)
    _require(completeness.get("ok") is True, "capability completeness is not valid")
    _require(completeness.get("current_milestone") == "multi_graph_retrieval_v1", "registry has not advanced to multi_graph_retrieval_v1")
    ...
    _require(
        (by_id.get("unified_context_namespace_v1") or {}).get("state") == "certified",
        "Unified Context Namespace must be certified before Multi-Graph admission",
    )
    _require(
        (by_id.get("multi_graph_retrieval_v1") or {}).get("state") in {"implemented", "verified"},
    ...
    _require("sqlite" not in source.casefold(), "Multi-Graph introduced its own SQLite persistence")
    _require("CanonicalRepositoryGraph" not in source, "Multi-Graph must consume graph views instead of subclassing repository authority")
    _require("ContextNamespaceAddress" in source, "Multi-Graph lost namespace identity integration")
    _require("GraphLayer" in source and "receipt_hash" in source, "Multi-Graph runtime surface drift")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Certify Syntavra Multi-Graph Retrieval v1")
