...
def advance() -> bool:
    ...
    if native != sorted(set(native)):
        raise RuntimeError(
            "native command inventory must be sorted and unique before fabric compact advance"
        )
    ...
    if not present:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"fabric compact inventory advance requires {BASE_COUNT} native routes, "
                f"got {len(native)}"
            )
        ...
        if len(native) < TARGET_COUNT:
            raise RuntimeError(
                "fabric compact inventory is present below its certified floor: "
    ...
    if len(native) < TARGET_COUNT or native != sorted(set(native)):
        raise RuntimeError(
            "fabric compact inventory must remain sorted, unique and at or above "
            "the 141-route certified floor"
        )
    if ROUTE not in native:
        raise RuntimeError("fabric compact is missing after inventory advance")
