...
def _host_detection(repo: Path, root: Path, fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(capabilities.get("platform"),str) or not isinstance(capabilities.get("python"),str) or not isinstance(capabilities.get("executables"),dict): raise AssertionError(f"environment capability schema drift: {capabilities}")
    return {"known_host_count":len(KNOWN_HOSTS),"known_host_sha256":hashlib.sha256("\n".join(sorted(KNOWN_HOSTS)).encode()).hexdigest(),"implicit_codex_equal":True,"codex":{"mode":codex["mode"],"enforced":codex["enforced"],"host":codex["capabilities"]["host"]},"claude_code":{"mode":claude["mode"],"enforced":claude["enforced"],"host":claude["capabilities"]["host"]},"detected_selected":{"codex_project_markers":list(by_host["codex"].get("project_markers") or []),"claude_user_markers":list(by_host["claude-code"].get("user_markers") or [])},"environment_schema_keys":sorted(capabilities),"platform_dynamic":capabilities["platform"],"python_dynamic":capabilities["python"]}
