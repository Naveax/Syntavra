...
def advance() -> bool:
    ...
    surface = document["rust_surface"]
    commands = list(surface["native_public_commands"])
    ...
    if ROUTE not in commands:
        if len(commands) != BASE_COUNT:
            raise RuntimeError(
                f"fabric installations requires {BASE_COUNT} prior native routes; "
        ...
        if len(commands) < TARGET_COUNT:
            raise RuntimeError("fabric installations is below its certified route floor")
        changed = False
    surface["native_public_commands"] = commands
    if changed:
        CONTRACT.write_text(
