...
def certify(repo: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="syntavra-memory-cert-") as td:
        ...
        sessions.open("session-cert", metadata={"user_id": "certifier"})
        ...
        _require(engine.recover(first["memory_id"], scope=scope).get("exact_recovery") is True, "memory recovery failed")
        _require(second["memory_id"] in engine.recover(first["memory_id"], scope=scope)["conflicts_with"], "conflict link lost")
