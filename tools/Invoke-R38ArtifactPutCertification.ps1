...
$repoRoot = Join-Path $WorkingRoot "Syntavra-r38-artifact-put-cert-$timestamp"
$validationScript = Join-Path $env:TEMP "r38-artifact-put-cert-$timestamp.sh"
...
git diff --binary > /tmp/artifact-put-first.diff
python tools/repair_r38_run_artifact_put_contract.py
python tools/advance_r38_run_artifact_put_inventory.py
cargo fmt --all
git diff --binary > /tmp/artifact-put-second.diff
cmp /tmp/artifact-put-first.diff /tmp/artifact-put-second.diff
...
assert rust["native_public_command_count"] == 167, rust
assert rust["missing_native_public_command_count"] == 78, rust
assert rust["native_coverage_ppm"] == 681632, rust
...
assert missing["native_public_command_count"] == 167, missing
assert missing["missing_native_public_command_count"] == 78, missing
assert "run artifact-put" not in missing["missing_native_public_commands"], missing

selector = Path("crates/syntavra-cli/src/bin/syntavra.rs").read_text(
    encoding="utf-8"
)
assert "const NATIVE_COMMAND_COUNT: u64 = 167;" in selector
...
cargo check --locked -p syntavra-cli --bin syntavra
git diff --check
printf '\nR38_ARTIFACT_PUT_CERTIFICATION_OK\n'
'@
...
if ([int]$contract.rust_surface.native_public_command_count -ne 167) {
    throw "Final native count 167 değil."
}
if ([int]$contract.rust_surface.missing_native_public_command_count -ne 78) {
    throw "Final missing count 78 değil."
}
...
if ($Push) {
    ...
    Invoke-Checked -FilePath "git" -ArgumentList @(
        ...
        "commit", "-m", "R38: certify native artifact put inventory"
    )
    Invoke-Checked -FilePath "git" -ArgumentList @(
        "-C", $repoRoot,
...
Write-Host "Native inventory target: 167/245; missing=78; coverage_ppm=681632; bridge=0" -ForegroundColor Green
