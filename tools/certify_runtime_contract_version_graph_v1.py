...
def certify(repo: Path) -> dict[str, Any]:
    ...
    _require(python_complete.get("ready") is True, "Runtime Contract Version Graph is post-completion Python hardening")
    _require(python_complete.get("rust_resume_allowed") is False, "Rust must remain retired/frozen during Python hardening")
    _require(python_complete.get("rust_retired") is True, "Rust retirement authority must remain explicit")
    _require(CAPABILITY_ID not in (registry.get("milestone_order") or []), "post-completion capability leaked into Python COMPLETE milestone order")
    post_completion_order = registry.get("post_completion_milestone_order") or []
    _require(CAPABILITY_ID in post_completion_order, "Runtime Contract Version Graph missing from post-completion milestone order")
    by_id = {item.get("id"): item for item in registry.get("capabilities") or [] if isinstance(item, dict)}
    lifecycle = by_id.get(CAPABILITY_ID) or {}
    lifecycle_state = str(lifecycle.get("state") or "")
    _require(lifecycle_state in {"implemented", "verified", "certified"}, f"invalid Runtime Contract Version Graph lifecycle: {lifecycle_state}")
    _require(lifecycle.get("required_for_python_complete") is False, "post-completion graph cannot reopen Python COMPLETE")
    post_completion_current_milestone = next(
        (milestone for milestone in post_completion_order if (by_id.get(milestone) or {}).get("state") != "certified"),
