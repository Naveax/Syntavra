...
def advance() -> bool:
    ...
    if native != sorted(set(native)):
        raise RuntimeError(
            "native command inventory must be sorted and unique before engine route advance"
        )
    ...
    if not present:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"engine route inventory advance requires {BASE_COUNT} native routes, "
                f"got {len(native)}"
            )
        ...
        if len(native) < TARGET_COUNT:
            raise RuntimeError(
                "engine route inventory is present below its certified floor: "
    ...
    if len(native) < TARGET_COUNT or native != sorted(set(native)):
        raise RuntimeError(
            "engine route inventory must remain sorted, unique and at or above "
            "the 138-route certified floor"
        )
    if ROUTE not in native:
        raise RuntimeError("generic engine route is missing after inventory advance")
