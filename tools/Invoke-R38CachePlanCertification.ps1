...
function Invoke-GitPushWithRetry {
    ...
    for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
        Write-Host "Push attempt $attempt/$MaxAttempts" -ForegroundColor DarkGray
        & git -C $WorkingDirectory push origin "HEAD:$RemoteBranch"
        $code = $LASTEXITCODE
        if ($code -eq 0) {
            return
        }
        if ($attempt -eq $MaxAttempts) {
            throw "Git push başarısız oldu (attempt=$attempt/$MaxAttempts, exit=$code). Validation clone korunuyor: $WorkingDirectory"
        }
        Write-Warning "Git push başarısız oldu (attempt=$attempt/$MaxAttempts, exit=$code). Yeniden deneniyor."
        Start-Sleep -Seconds (5 * $attempt)
    }
}
...
$repoRoot = Join-Path $WorkingRoot "Syntavra-r38-cache-plan-cert-$timestamp"
$validationScript = Join-Path $env:TEMP "r38-cache-plan-cert-$timestamp.sh"

Write-Host "`n=== TEMİZ EXACT-HEAD CLONE ===" -ForegroundColor Cyan
Invoke-GitCloneWithRetry -Remote $Repository -RemoteBranch $Branch -Destination $repoRoot
...
python tools/repair_r38_run_cache_plan_contract.py
python tools/advance_r38_run_cache_plan_inventory.py
cargo fmt --all
cargo check --locked -p syntavra-cli --bin syntavra
git diff --binary > /tmp/cache-plan-first.diff
python tools/repair_r38_run_cache_plan_contract.py
python tools/advance_r38_run_cache_plan_inventory.py
cargo fmt --all
git diff --binary > /tmp/cache-plan-second.diff
cmp /tmp/cache-plan-first.diff /tmp/cache-plan-second.diff
...
SYNTAVRA_R38_SELECTOR="$PWD/target/debug/syntavra" \
  python -m pytest -q \
    ...
    tests/runtime/test_native_run_cache_plan_r38.py
...
reproduce_metadata() {
  ...
  python tools/repair_r38_run_artifact_verify_contract.py
  python tools/advance_r38_run_artifact_verify_inventory.py
  python tools/repair_r38_run_cache_plan_contract.py
  python tools/advance_r38_run_cache_plan_inventory.py
  python tools/repair_r38_native_expansion_sync.py
...
contract = json.loads(Path("contracts/engine/dual-engine-public-surface-v2.json").read_text(encoding="utf-8"))
rust = contract["rust_surface"]
assert contract["python_surface"]["public_command_count"] == 245, contract
assert rust["native_public_command_count"] == 171, rust
assert rust["missing_native_public_command_count"] == 74, rust
assert rust["native_coverage_ppm"] == 697959, rust
...
for route in (
    ...
    assert route in rust["native_public_commands"], (route, rust)

missing = json.loads(Path("contracts/engine/r38-missing-native-commands-v1.json").read_text(encoding="utf-8"))
assert missing["python_public_command_count"] == 245, missing
assert missing["native_public_command_count"] == 171, missing
assert missing["missing_native_public_command_count"] == 74, missing
assert missing["python_launcher_bridge_command_count"] == 0, missing
assert "run cache-plan" not in missing["missing_native_public_commands"], missing

selector = Path("crates/syntavra-cli/src/bin/syntavra.rs").read_text(encoding="utf-8")
assert "const NATIVE_COMMAND_COUNT: u64 = 171;" in selector

manifest = Path("MANIFEST.sha256").read_text(encoding="utf-8")
for stale in (
    ".r38-evidence-rotate-key-certify.sh",
    ".r38-artifact-put",
    ".r38-artifact-query",
    ".r38-artifact-stats",
    ".r38-artifact-verify",
...
print(json.dumps({"ok": True, "native": 171, "missing": 74, "coverage_ppm": 697959, "bridge": 0}, sort_keys=True))
PY

cargo check --locked -p syntavra-cli --bin syntavra
git diff --check
printf '\nR38_CACHE_PLAN_CERTIFICATION_OK\n'
'@
...
if ([int]$contract.rust_surface.native_public_command_count -ne 171) { throw "Final native count 171 değil." }
if ([int]$contract.rust_surface.missing_native_public_command_count -ne 74) { throw "Final missing count 74 değil." }
if ([int]$contract.rust_surface.native_coverage_ppm -ne 697959) { throw "Final coverage ppm 697959 değil." }
if ([int]$contract.rust_surface.python_launcher_bridge_command_count -ne 0) { throw "Bridge count sıfır değil." }

Write-Host "`nGenerated diff:" -ForegroundColor Green
& git -C $repoRoot --no-pager diff --stat
if ($LASTEXITCODE -ne 0) { throw "git diff --stat başarısız oldu." }
...
if ($Push) {
    ...
    Invoke-Checked -FilePath "git" -ArgumentList @(
        ...
        "commit", "-m", "R38: certify native cache plan inventory"
    )
    Invoke-GitPushWithRetry -WorkingDirectory $repoRoot -RemoteBranch $Branch
...
Write-Host "Native inventory target: 171/245; missing=74; coverage_ppm=697959; bridge=0" -ForegroundColor Green
