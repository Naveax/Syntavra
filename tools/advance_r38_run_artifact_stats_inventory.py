...
def advance() -> bool:
    ...
    if ROUTE not in native:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"artifact stats advance requires {BASE_COUNT} native routes, got {len(native)}"
            )
        native = sorted([*native, ROUTE])
        changed = True
    else:
        if len(native) < TARGET_COUNT:
            raise RuntimeError("artifact stats present below certified floor")
        changed = False
