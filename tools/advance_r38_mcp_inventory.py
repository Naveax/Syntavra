...
def advance() -> bool:
    ...
    if native != sorted(set(native)):
        raise RuntimeError("native command inventory must be sorted and unique before MCP advance")

    present = ROUTE in native
    if not present:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"MCP inventory advance requires {BASE_COUNT} native routes, got {len(native)}"
            )
        ...
        if len(native) < TARGET_COUNT:
            raise RuntimeError(
                f"MCP inventory is present below its certified floor: "
    ...
    if len(native) < TARGET_COUNT or native != sorted(set(native)):
        raise RuntimeError(
            "MCP inventory must remain sorted, unique and at or above the 137-route certified floor"
        )
    if ROUTE not in native:
        raise RuntimeError("MCP certified route is missing after inventory advance")
