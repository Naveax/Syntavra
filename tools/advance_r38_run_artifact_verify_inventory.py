...
def advance() -> bool:
    ...
    if python["public_command_count"] != PUBLIC_COUNT:
        raise RuntimeError(
            f"artifact verify advance requires {PUBLIC_COUNT} Python routes, "
            f"got {python['public_command_count']}"
        )
    if rust["python_launcher_bridge_command_count"] != 0:
        raise RuntimeError("artifact verify advance forbids Python launcher bridge routes")
    ...
    if ROUTE not in native:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"artifact verify advance requires {BASE_COUNT} native routes, got {len(native)}"
            )
        native = sorted([*native, ROUTE])
        changed = True
    else:
        if len(native) < TARGET_COUNT:
            raise RuntimeError("artifact verify present below certified floor")
        changed = False
