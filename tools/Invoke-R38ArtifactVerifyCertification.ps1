...
$repoRoot = Join-Path $WorkingRoot "Syntavra-r38-artifact-verify-cert-$timestamp"
$validationScript = Join-Path $env:TEMP "r38-artifact-verify-cert-$timestamp.sh"
...
git diff --binary > /tmp/artifact-verify-first.diff
python tools/repair_r38_run_artifact_verify_contract.py
python tools/advance_r38_run_artifact_verify_inventory.py
cargo fmt --all
git diff --binary > /tmp/artifact-verify-second.diff
cmp /tmp/artifact-verify-first.diff /tmp/artifact-verify-second.diff
...
assert rust["native_public_command_count"] == 170, rust
assert rust["missing_native_public_command_count"] == 75, rust
assert rust["native_coverage_ppm"] == 693877, rust
assert rust["python_launcher_bridge_command_count"] == 0, rust
for route in ("run artifact-put", "run artifact-query", "run artifact-stats", "run artifact-verify"):
...
assert missing["native_public_command_count"] == 170, missing
assert missing["missing_native_public_command_count"] == 75, missing
assert missing["python_launcher_bridge_command_count"] == 0, missing
assert "run artifact-verify" not in missing["missing_native_public_commands"], missing

selector = Path("crates/syntavra-cli/src/bin/syntavra.rs").read_text(encoding="utf-8")
assert "const NATIVE_COMMAND_COUNT: u64 = 170;" in selector
...
print(json.dumps({"ok": True, "native": 170, "missing": 75, "coverage_ppm": 693877, "bridge": 0}, sort_keys=True))
PY

cargo check --locked -p syntavra-cli --bin syntavra
git diff --check
printf '\nR38_ARTIFACT_VERIFY_CERTIFICATION_OK\n'
'@
...
if ([int]$contract.rust_surface.native_public_command_count -ne 170) { throw "Final native count 170 değil." }
if ([int]$contract.rust_surface.missing_native_public_command_count -ne 75) { throw "Final missing count 75 değil." }
if ([int]$contract.rust_surface.native_coverage_ppm -ne 693877) { throw "Final coverage ppm 693877 değil." }
...
if ($Push) {
    ...
    Invoke-Checked -FilePath "git" -ArgumentList @(
        ...
        "commit", "-m", "R38: certify native artifact verify inventory"
    )
...
Write-Host "Native inventory target: 170/245; missing=75; coverage_ppm=693877; bridge=0" -ForegroundColor Green
