...
def extract_outer_yaml_python_block(text: str) -> str:
    lines = text.splitlines()
    start_marker = "          python - <<'PY'"
    end_marker = "          PY"
    next_outer_step = "      - name: Set up Python"
    try:
        start = lines.index(start_marker) + 1
    except ValueError as exc:
        raise RuntimeError('outer helper apply block start marker missing') from exc
    try:
        boundary = lines.index(next_outer_step, start)
    except ValueError as exc:
        raise RuntimeError('outer helper Set up Python boundary missing') from exc
    candidates = [index for index in range(start, boundary) if lines[index] == end_marker]
    if not candidates:
        raise RuntimeError('outer helper apply block end marker missing')
    end = candidates[-1]
    prefix = ' ' * 10
    raw = lines[start:end]
...
def main() -> None:
    ...
    script = extract_outer_yaml_python_block(helper_text)
    compile(script, str(HELPER), 'exec')
