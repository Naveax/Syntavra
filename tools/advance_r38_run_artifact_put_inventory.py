...
def advance() -> bool:
    ...
    if ROUTE not in native:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"artifact put advance requires {BASE_COUNT} native routes, got {len(native)}"
            )
        native = sorted([*native, ROUTE])
        changed = True
    else:
        if len(native) < TARGET_COUNT:
            raise RuntimeError("artifact put present below certified floor")
        changed = False
