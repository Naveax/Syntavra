...
$repoRoot = Join-Path $WorkingRoot "Syntavra-r38-artifact-query-cert-$timestamp"
$validationScript = Join-Path $env:TEMP "r38-artifact-query-cert-$timestamp.sh"
...
git diff --binary > /tmp/artifact-query-first.diff
python tools/repair_r38_run_artifact_query_contract.py
python tools/advance_r38_run_artifact_query_inventory.py
cargo fmt --all
git diff --binary > /tmp/artifact-query-second.diff
cmp /tmp/artifact-query-first.diff /tmp/artifact-query-second.diff
...
assert rust["native_public_command_count"] == 168, rust
assert rust["missing_native_public_command_count"] == 77, rust
assert rust["native_coverage_ppm"] == 685714, rust
...
assert missing["native_public_command_count"] == 168, missing
assert missing["missing_native_public_command_count"] == 77, missing
assert missing["python_launcher_bridge_command_count"] == 0, missing
assert "run artifact-query" not in missing["missing_native_public_commands"], missing

selector = Path("crates/syntavra-cli/src/bin/syntavra.rs").read_text(
    encoding="utf-8"
)
assert "const NATIVE_COMMAND_COUNT: u64 = 168;" in selector
...
cargo check --locked -p syntavra-cli --bin syntavra
git diff --check
printf '\nR38_ARTIFACT_QUERY_CERTIFICATION_OK\n'
'@
...
if ([int]$contract.rust_surface.native_public_command_count -ne 168) {
    throw "Final native count 168 değil."
}
if ([int]$contract.rust_surface.missing_native_public_command_count -ne 77) {
    throw "Final missing count 77 değil."
}
if ([int]$contract.rust_surface.native_coverage_ppm -ne 685714) {
    throw "Final coverage ppm 685714 değil."
}
...
if ($Push) {
    ...
    Invoke-Checked -FilePath "git" -ArgumentList @(
        ...
        "commit", "-m", "R38: certify native artifact query inventory"
    )
    Invoke-Checked -FilePath "git" -ArgumentList @(
        "-C", $repoRoot,
...
Write-Host "Native inventory target: 168/245; missing=77; coverage_ppm=685714; bridge=0" -ForegroundColor Green
