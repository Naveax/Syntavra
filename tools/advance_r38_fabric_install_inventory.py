...
def advance() -> bool:
    ...
    if native != sorted(set(native)):
        raise RuntimeError(
            "native command inventory must be sorted and unique before fabric install advance"
        )

    if ROUTE not in native:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"fabric install inventory advance requires {BASE_COUNT} native routes, "
                f"got {len(native)}"
            )
        ...
        if len(native) < TARGET_COUNT:
            raise RuntimeError(
                "fabric install inventory is present below its certified floor: "
                f"native count is {len(native)}, minimum {TARGET_COUNT}"
            )
        changed = False

    if ROUTE not in native or len(native) < TARGET_COUNT:
        raise RuntimeError("fabric install inventory advance failed")
