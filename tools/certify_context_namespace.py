    ...
    return UniversalContextItem.build(
        ...
        recovery=(
            RecoveryHandle(
                kind="file-range",
                locator={
                    "path": "syntavra_runtime/context_namespace.py",
                    "start_line": 1,
                    "end_line": 40,
                },
...
def _runtime_smoke() -> dict[str, Any]:
    ...
    uris = {
        ...
        "dir": ContextNamespaceAddress.repository(
    ...
    _require(
        "exact namespace payload symbol" not in json.dumps(l2, sort_keys=True),
        "L2 leaked scalar payload",
    )
    _require((l2.get("structure") or {}).get("type") == "object", "L2 structure drift")

    l3 = namespace.reveal(uris["symbol"], level="L3")["view"]
    _require(
        (l3.get("content") or {}).get("text") == "exact namespace payload symbol",
        "L3 exact reveal drift",
    )
    ...
    for parent_key, child_key in chain:
        ...
        _require(
            (view.get("children") or [{}])[0].get("uri") == uris[child_key],
            f"{parent_key}->{child_key} descent drift",
        )
    ...
    trajectory = namespace.start_trajectory(
        "certify progressive context navigation",
        root_uri=uris["repo"],
    )
    ...
    _require(
        [row["sequence"] for row in receipt.get("steps", [])] == [1, 2, 3],
        "trajectory sequence drift",
    )
    _require(
        [row["operation"] for row in receipt.get("steps", [])]
        == ["browse", "why", "reveal"],
        "trajectory operation drift",
    )
...
def _validate_workflow_pins(workflow: str) -> None:
    ...
    _require(
        ...
        f"Context Namespace external action set drift: {sorted(external)}",
    )
    for ref in external:
...
def _validate_enforcement(repo: Path) -> dict[str, str]:
    for relative in (WORKFLOW_RELATIVE, RELEASE_GATE_RELATIVE, TEST_RELATIVE):
        _require(
            ...
            f"missing Context Namespace enforcement surface: {relative.as_posix()}",
        )

    workflow = (repo / WORKFLOW_RELATIVE).read_text(encoding="utf-8")
    _require(
        "group: unified-context-namespace-${{ github.event.pull_request.number || github.ref }}"
        in workflow,
        "Context Namespace concurrency is not PR/ref scoped",
    )
    _require(
        "tests.runtime.test_context_namespace" in workflow,
        "Context Namespace workflow lost regression suite",
    )
    _require(
        "tools/certify_context_namespace.py" in workflow,
        "Context Namespace workflow lost certifier",
    )
    _validate_workflow_pins(workflow)

    # Release Main remains an independent protected co-gate. This milestone does
    # not mutate that trust chain or the central action-pin policy.
...
def certify(repo: Path) -> dict[str, Any]:
    ...
    _require(
        repo == ROOT,
        f"Context Namespace certifier must run against its own checkout: {repo} != {ROOT}",
    )
    ...
    _require(
        contract.get("content_authority")
        == "syntavra_runtime/universal_context_item.py:UniversalContextItem",
        "UniversalContextItem authority drift",
    )
    _require(contract.get("scheme") == "syntavra", "syntavra:// scheme drift")
    _require(
        contract.get("progressive_repository_descent")
        == ["repo", "directory", "file", "symbol", "lines"],
        "progressive repository descent drift",
    )
    ...
    _require(
        namespace_policy.get("public_cli_route_added") is False,
        "Context Namespace invented a public CLI route",
    )
    ...
    for key in (
        ...
        _require(
            trajectory_policy.get(key) is True,
            f"Context trajectory policy disabled: {key}",
        )
    ...
    _require(
        (rust_freeze.get("rust") or {}).get("remaining_parity_promotion") == 71,
    ...
    _require(
        (by_id.get("deferred_tool_discovery_v1") or {}).get("state") == "certified",
        "Deferred Tool Discovery must be certified before Context Namespace admission",
    )
    _require(
    ...
    _require(
        "ContextNamespaceAddress" in source and "trajectory_receipt" in source,
        "Context Namespace runtime surface drift",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Certify Syntavra Unified Context Namespace v1"
    )
