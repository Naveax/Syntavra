...
def advance() -> bool:
    ...
    if ROUTE not in native:
        if len(native) != BASE_COUNT:
            raise RuntimeError(
                f"fabric rollback-install advance requires {BASE_COUNT} native routes, got {len(native)}"
            )
        native = sorted([*native, ROUTE])
        changed = True
    else:
        if len(native) < TARGET_COUNT:
            raise RuntimeError("fabric rollback-install present below certified floor")
        changed = False
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "native_public_command_count": len(contract["rust_surface"]["native_public_commands"]),
        "ok": True,
        "route": ROUTE,
    }, sort_keys=True))
    return 0
