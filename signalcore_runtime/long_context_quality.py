...
class LongContextReceipt:
    ...
    def validate(self) -> tuple[str, ...]:
        ...
        if self.arm not in {"baseline", "signalcore"}:
...
class LongContextQualityGate:
    ...
    def evaluate(cls, receipts: Iterable[LongContextReceipt]) -> dict[str, Any]:
        ...
        pairs = [group for group in groups.values() if "baseline" in group and "signalcore" in group]
        if len(pairs) < cls.minimum_pairs:
            reasons.append("insufficient-paired-runs")

        cases = {pair["signalcore"].case_id for pair in pairs}
        families = {pair["signalcore"].task_family for pair in pairs}
        tiers = {pair["signalcore"].tier_tokens for pair in pairs}
        ...
        for pair in pairs:
            ...
            if baseline.provider != signalcore.provider or baseline.model != signalcore.model:
                ...
                continue
            quality_deltas.append(signalcore.answer_quality - baseline.answer_quality)
            recalls.append(signalcore.required_fact_recall)
            stale_rejections.append(signalcore.stale_fact_rejection)
            precisions.append(signalcore.evidence_precision)
            baseline_tokens = baseline.input_tokens + baseline.output_tokens
            signalcore_tokens = signalcore.input_tokens + signalcore.output_tokens
            ...
            if not signalcore.exact_recovery:
                reasons.append("exact-recovery-failed")
            if signalcore.forced_restart:
                reasons.append("forced-restart-observed")
            if signalcore.task_family == "cross-session-continuity" and not signalcore.continuity_restored:
...
def manifest() -> dict[str, Any]:
    return {
        "version": VERSION,
        "channel": CHANNEL,
        "name": "SignalCore Long-Context Quality Protocol",
