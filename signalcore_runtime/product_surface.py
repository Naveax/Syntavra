...
MENTAL_MODEL: tuple[ProductMentalModel, ...] = (
    ...
    ProductMentalModel("run", "enforce routing and execute through SignalCore", "auditable execution plan"),
...
MCP_PROFILES: dict[str, MCPProfile] = {
    "minimal": MCPProfile(
        "minimal",
        ("signalcore.search", "signalcore.read", "signalcore.run", "signalcore.restore"),
        4,
        700,
        120,
        True,
    ...
    "balanced": MCPProfile(
        ...
        (
            "signalcore.search", "signalcore.read", "signalcore.run", "signalcore.restore",
            "signalcore.session", "signalcore.metrics", "signalcore.sandbox", "signalcore.proof",
        ),
        8,
        1400,
        180,
    ...
    "audit": MCPProfile(
        ...
        (
            "signalcore.search", "signalcore.read", "signalcore.run", "signalcore.restore",
            "signalcore.session", "signalcore.metrics", "signalcore.sandbox", "signalcore.proof",
            "signalcore.evidence", "signalcore.config", "signalcore.backup", "signalcore.migrate",
        ),
        12,
        2600,
        300,
...
PLATFORM_ADAPTERS: tuple[PlatformAdapter, ...] = (
    ...
    PlatformAdapter("cursor", ("cursor",), (".cursor/rules/signalcore.mdc", ".cursor/mcp.json"), "rules+mcp", True, False, True),
    ...
    PlatformAdapter("roo-code", (), (".roo/rules/signalcore.md", ".vscode/mcp.json"), "rules+mcp", True, False, True),
    PlatformAdapter("qwen-code", ("qwen", "qwen-code"), ("QWEN.md", "~/.qwen/settings.json"), "agents+mcp", True, False, True),
    PlatformAdapter("kiro", (), (".kiro/steering/signalcore.md", ".kiro/settings/mcp.json"), "steering+mcp", True, False, True),
...
class ProviderUsageReceipt:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ProviderUsageReceipt":
        return cls(
            ...
            arm=str(value.get("arm", "signalcore")),
    ...
    def validate(self) -> tuple[str, ...]:
        ...
        if self.arm not in {"baseline", "signalcore", "token-savior", "context-mode", "headroom", "volt-lcm"}:
...
class MeasuredBenchmarkGate:
    ...
    def evaluate(cls, receipts: Iterable[ProviderUsageReceipt]) -> dict[str, Any]:
        ...
        repositories = {pair["signalcore"].repository_hash for pair in pairs}
        tasks = {pair["signalcore"].task_id for pair in pairs}
        workloads = {pair["signalcore"].workload for pair in pairs}
...
def write_receipt_schema(path: Path) -> dict[str, Any]:
    schema = {
        ...
        "$id": "https://signalcore.dev/schemas/provider-usage-receipt-v1.json",
        "title": "SignalCore Provider Usage Receipt",
        "type": "object",
        "required": [
        ...
        "properties": {
            ...
            "arm": {"enum": ["baseline", "signalcore", "token-savior", "context-mode", "headroom", "volt-lcm"]},
