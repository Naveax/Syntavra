...
class SignalCoreError(RuntimeError):
...
def error_descriptor(code: str, **details: Any) -> ErrorDescriptor:
    ...
    if base is None:
        return ErrorDescriptor(
            ...
            message="Unhandled SignalCore error",
            fatal=True,
            details=details,
        )
    return ErrorDescriptor(
...
def raise_error(code: str, *, cause: BaseException | None = None, **details: Any) -> None:
    raise SignalCoreError(error_descriptor(code, **details), cause=cause)


for _descriptor in (
    ErrorDescriptor("SC_CONFIG_INVALID", ErrorCategory.CONFIG, "SignalCore configuration is invalid", False, True, 400),
