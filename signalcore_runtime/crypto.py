...
def derive_key(master_key: bytes, *, project_id: str, key_id: str) -> bytes:
    ...
    salt = hashlib.sha256(("signalcore:" + project_id).encode("utf-8")).digest()
...
class KeyRing:
    ...
    Managed deployments should set SIGNALCORE_EVIDENCE_MASTER_KEY_B64 and disable
    ...
    def __init__(
        ...
        *,
        project_id: str,
        env_name: str = "SIGNALCORE_EVIDENCE_MASTER_KEY_B64",
    ...
    def rotate(self) -> str:
        if os.environ.get(self.env_name):
            raise CryptoError("environment-managed keys must be rotated outside SignalCore")
        registry = self._load_registry()
