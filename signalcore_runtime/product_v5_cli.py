...
def _state(args: argparse.Namespace) -> Path:
    return Path(args.state_root).resolve(strict=False) if args.state_root else _project(args) / ".signalcore" / "runtime-v3"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="signalcore-product")
    ...
    for name in ("plan", "install", "verify", "uninstall"):
        ...
        item.add_argument("--name", default="signalcore-provider-proxy")
        item.add_argument("--platform", choices=("linux", "darwin", "windows"))
        item.add_argument("--home"); item.add_argument("--environment-file", default="")
        item.add_argument("--working-directory", default=""); item.add_argument("--description", default="SignalCore provider proxy")
