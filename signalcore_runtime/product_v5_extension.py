...
_PRODUCT_NAMES = frozenset({
    "signalcore.product.capabilities",
    "signalcore.data.route",
    "signalcore.policy.record",
    "signalcore.policy.recommend",
    "signalcore.policy.active",
    "signalcore.service.plan",
    "signalcore.service.verify",
    "signalcore.arm.validate_result",
})
...
def product_tools() -> list[dict[str, Any]]:
    return [
        _tool("signalcore.product.capabilities", "Inspect product-parity V5 data, policy, service, SDK and arm-runner surfaces"),
        _tool(
            "signalcore.data.route",
...
def install() -> None:
    ...
    if getattr(MCPServer, "_signalcore_product_extension_v5", False):
        return
    ...
    def extended_call_tool(self: Any, name: str, arguments: dict[str, Any]) -> Any:
        if name == "signalcore.product.capabilities":
            return {
                "schema_version": 1,
        ...
        if name == "signalcore.policy.record":
        ...
        if name == "signalcore.policy.recommend":
        ...
        if name == "signalcore.policy.active":
            return self.policy_tuner.active(
                str(arguments["family"]), host=str(arguments.get("host", "unknown")),
                model=str(arguments.get("model", "unknown")),
            ) or {"ok": False, "reason": "no-promoted-policy"}
        if name in {"signalcore.service.plan", "signalcore.service.verify"}:
            spec = ServiceSpec(
        ...
        if name == "signalcore.arm.validate_result":
    ...
    MCPServer._signalcore_product_extension_v5 = True
