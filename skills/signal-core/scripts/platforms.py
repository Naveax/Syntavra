...
def compact_adapter(platform_id: str = "generic") -> str:
    return f"""{BEGIN}
## SignalCore {PUBLIC_VERSION}

For non-trivial coding-agent tasks, use SignalCore as a success-first context and evidence controller.

1. Read the canonical skill at `skills/signal-core/SKILL.md` or the installed platform-native copy.
...
5. Run the standard-library scripts under `skills/signal-core/scripts/` when the host cannot invoke Agent Skills natively.
...
def cursor_adapter() -> str:
    ...
    return "---\ndescription: SignalCore success-first context, evidence, and cost routing\nglobs:\nalwaysApply: false\n---\n\n" + body + "\n"
...
def _copy_skill_atomic(destination: Path, *, force: bool = False) -> dict[str, Any]:
    ...
    stage = Path(tempfile.mkdtemp(prefix=".signal-core-stage-", dir=destination.parent))
    backup: Path | None = None
    try:
        shutil.copytree(SKILL_ROOT, stage / "signal-core", symlinks=False, dirs_exist_ok=False)
        staged = stage / "signal-core"
        ...
        if destination.exists():
            ...
            backup = destination.with_name(destination.name + ".signalcore-backup")
            if backup.exists():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SignalCore cross-platform adapter and installer")
