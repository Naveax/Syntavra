...
def install() -> None:
    ...
    if getattr(SandboxManager, "_syntavra_v6_hardening", False):
        return
    ...
    def plan(
        ...
        *,
        ...
        configured_image = str(overrides.get("SYNTAVRA_SANDBOX_IMAGE") or "")
        selected, _ = self.select_backend(policy)
        if selected in {"docker", "podman"}:
            if not configured_image:
                raise SandboxError(
                    "container sandbox requires SYNTAVRA_SANDBOX_IMAGE pinned by digest"
                )
            ...
            overrides["SYNTAVRA_SANDBOX_IMAGE"] = configured_image
    ...
    SandboxManager._syntavra_v6_hardening = True
