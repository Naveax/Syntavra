...
class ProxyConfig:
    ...
    control_token_env: str = "SYNTAVRA_PROXY_CONTROL_TOKEN"
...
class ProviderProxyRuntime:
    ...
    def _handler_type(self) -> type[BaseHTTPRequestHandler]:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            server_version = "SyntavraProviderProxy/0.6"
            ...
            def do_GET(self) -> None:
                ...
                if target == "/_syntavra/health":
                    if self._control():
                        self._json(HTTPStatus.OK, runtime.status())
                    return
                if target == "/_syntavra/ready":
                    if self._control():
    ...
    def start(self) -> tuple[str, int]:
        ...
        self._thread = threading.Thread(target=self._server.serve_forever, name="syntavra-provider-proxy", daemon=True)
        self._thread.start()
        return self.address
