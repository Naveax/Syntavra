...
def decide_claim(
    *,
    ...
    syntavra = list(syntavra_costs)
    reasons: list[str] = []
    if not actual_quota_available:
        reasons.append("actual-quota-unavailable")
    if len(baseline) != len(syntavra) or not baseline:
    ...
    ratios = [base / signal for base, signal in zip(baseline, syntavra) if base > 0 and signal > 0]
