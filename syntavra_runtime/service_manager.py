...
class ServiceSpec:
    ...
    description: str = "Syntavra provider proxy"
...
class ProviderProxyServiceManager:
    ...
    def plan(self, spec: ServiceSpec, *, platform_name: str | None = None) -> ServicePlan:
        ...
        if platform_name == "linux":
            ...
            label = f"dev.syntavra.{spec.name}"
            ...
            path = self.home / "AppData" / "Local" / "Syntavra" / "services" / f"{spec.name}.xml"
    ...
    def _launchd(spec: ServiceSpec, label: str) -> str:
        ...
        if spec.environment_file:
            payload["EnvironmentVariables"] = {"SYNTAVRA_ENV_FILE": spec.environment_file}
