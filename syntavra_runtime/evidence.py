...
class EvidenceKeyring:
    ...
    Keys may be injected through ``SYNTAVRA_EVIDENCE_KEY`` (base64 or hex) for
    ...
    def _key_from_env() -> bytes | None:
        raw = os.environ.get("SYNTAVRA_EVIDENCE_KEY", "").strip()
        if not raw:
            return None
        try:
            ...
            raise EvidenceError("SYNTAVRA_EVIDENCE_KEY is not valid hex/base64") from exc
    ...
    def data_key(self, version: int) -> bytes:
        salt = hashlib.sha256(("syntavra:evidence:" + self.project_id).encode("utf-8")).digest()
        return HKDF(
            algorithm=hashes.SHA256(),
            length=_KEY_BYTES,
            salt=salt,
            info=f"syntavra-evidence-v{version}".encode("ascii"),
