...
class HostInstaller:
    ...
    def _render_config(self, host: str, existing: dict[str, Any]) -> dict[str, Any]:
        ...
        if spec.supports_pre_tool_hook or spec.supports_post_tool_hook:
            ...
            if spec.supports_pre_tool_hook:
                entries = [row for row in hooks.get("PreToolUse", []) if row.get("syntavraManaged") is not True]
                entries.append({**self._hook_entry("pre"), "syntavraManaged": True})
                hooks["PreToolUse"] = entries
            if spec.supports_post_tool_hook:
                entries = [row for row in hooks.get("PostToolUse", []) if row.get("syntavraManaged") is not True]
                entries.append({**self._hook_entry("post"), "syntavraManaged": True})
                hooks["PostToolUse"] = entries
        value.setdefault("syntavra", {}).update({
            "managed": True,
            "version": VERSION,
    ...
    def _install_instruction(self, host: str, *, scope: str) -> InstallChange | None:
        ...
        block = (
            f"{marker_start}\n"
            "# Syntavra\n"
            "Use the local Syntavra pre-release runtime before broad repository reads, long-running commands, "
            "large tool outputs, context compaction, or verifier reuse. Run `syntavra doctor` when enforcement is uncertain.\n"
            f"{marker_end}\n"
        )
