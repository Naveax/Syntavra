...
from .telemetry_metrics_contract import (
    CAPABILITY,
    INPUT_FORMAT,
    INPUT_PROFILE,
    ROUTE,
...
class TelemetryMetricsRouterR24(MigrationPlanRouterR24):
    """R24 routing for process-local no-write telemetry metrics output."""

    @staticmethod
    def supported_commands() -> tuple[str, ...]:
        return tuple(sorted((*MigrationPlanRouterR24.supported_commands(), ROUTE)))
    ...
    def route(
        ...
        *,
        ...
        scheduler_states: tuple[str, ...] | None = None,
        scheduler_limit: int | None = None,
        migration_database: str | None = None,
        telemetry_prometheus: bool = False,
    ) -> dict[str, Any]:
        ...
        if any(
            (
                ...
                explain_path is not None,
                scheduler_states is not None,
                scheduler_limit is not None,
                migration_database is not None,
            )
        ):
            raise EngineSelectionError(
                "ENGINE_ROUTE_TELEMETRY_INPUT_UNSUPPORTED_R24",
                "telemetry.metrics accepts only the output format flag",
                phase=ROUTING_PHASE,
        ...
        output_format = "prometheus" if telemetry_prometheus else "json"
        try:
            expected = telemetry_metrics_result(output_format)
            request_bytes = canonical_request_bytes(output_format)
            arguments = rust_argv(output_format)
        except TelemetryMetricsError as exc:
            raise EngineSelectionError(
                "ENGINE_ROUTE_TELEMETRY_PREFLIGHT_FAILED_R24",
                "Telemetry metrics preflight failed before engine selection",
                phase=ROUTING_PHASE,
                schema_version=ROUTING_SCHEMA_VERSION,
                command=ROUTE,
                reason=str(exc),
        ...
        if selection.resolved == "rust":
            ...
            if not verification.compatible or CAPABILITY not in verification.capabilities:
                raise EngineSelectionError(
                    ...
                    "Rust is selected but telemetry.metrics is not verified",
                    phase=ROUTING_PHASE,
                    schema_version=ROUTING_SCHEMA_VERSION,
                    command=ROUTE,
                    capability=CAPABILITY,
            ...
            try:
                ...
                raise EngineSelectionError(
                    ...
                    "Rust telemetry.metrics failed and was not re-executed in Python",
                    phase=ROUTING_PHASE,
                    schema_version=ROUTING_SCHEMA_VERSION,
                    command=ROUTE,
                    capability=CAPABILITY,
            ...
            if candidate != expected:
                ...
                raise EngineSelectionError(
                    "RUST_TELEMETRY_METRICS_PARITY_INVALID_R24",
                    "Rust telemetry.metrics differs from the Python canonical result",
                    phase=ROUTING_PHASE,
                    schema_version=ROUTING_SCHEMA_VERSION,
                    command=ROUTE,
                    capability=CAPABILITY,
        ...
        if response_bytes > MAX_RESPONSE_BYTES:
            raise EngineSelectionError(
                ...
                "The telemetry metrics result exceeded the response limit",
                phase=ROUTING_PHASE,
                schema_version=ROUTING_SCHEMA_VERSION,
                command=ROUTE,
...
__all__ = ["TelemetryMetricsRouterR24"]
