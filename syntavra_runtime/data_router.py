    ...
    def _fit_envelope(
        ...
        *,
        ...
        def envelope(payload: Any, *, truncated: bool) -> dict[str, Any]:
            ...
            value: dict[str, Any] = {"_syntavra": meta, "data": payload}
            # V5 compatibility: structured fields remain directly addressable.
            if isinstance(payload, Mapping):
                for key, item in payload.items():
                    if key not in value and key != "_syntavra":
                        value[key] = item
            return value
        ...
        if len(text.encode("utf-8")) > policy.budget_bytes:
            minimal = {
                "_syntavra": {"schema_version": self.schema_version, "truncated": True, "sha256": exact_hash},
                "data": None,
            }
            text = self._encode(minimal)
        if len(text.encode("utf-8")) > policy.budget_bytes:
