...
#[path = "native_adapter_configure.rs"]
...
#[path = "native_fabric_doctor.rs"]
mod native_fabric_doctor;
#[path = "native_fabric_insights.rs"]
...
#[path = "native_fabric_platform_plan.rs"]
...
pub fn execute(
    ...
    if native_fabric_insights::supports(command) {
        return native_fabric_insights::execute(&arguments, state_root).map(Some);
    }
    if native_fabric_profile::supports(command) {
    ...
    if native_backup_verify::supports(command) {
    ...
    if native_compress_get::supports(command) {
