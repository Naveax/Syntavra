...
fn project_root(project_root: &str, expected_project_id: &str) -> Result<PathBuf> {
    ...
    let actual = project_id_for_root(project_root).map_err(|code| {
        FullParityFailure::new(format!("FULL_PARITY_{}", code.trim_start_matches("STATE_")))
    })?;
...
fn state_root(root: &Path) -> Result<PathBuf> {
    ...
    fs::create_dir_all(&state)
        .map_err(|_| FullParityFailure::new("FULL_PARITY_STATE_CREATE_FAILED"))?;
...
fn atomic_write(path: &Path, payload: &[u8]) -> Result<()> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_TARGET_PARENT_INVALID"))?;
    fs::create_dir_all(parent)
        .map_err(|_| FullParityFailure::new("FULL_PARITY_ATOMIC_WRITE_FAILED"))?;
    reject_symlink(parent, "FULL_PARITY_TARGET_PARENT_SYMLINK")?;
    let name = path
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_TARGET_NAME_INVALID"))?;
...
fn read_json(path: &Path, default: Value) -> Result<Value> {
    ...
    let raw =
        fs::read(path).map_err(|_| FullParityFailure::new("FULL_PARITY_STATE_READ_FAILED"))?;
    if raw.len() > MAX_REQUEST_BYTES {
        return failure("FULL_PARITY_STATE_TOO_LARGE");
    }
    serde_json::from_slice(&raw)
        .map_err(|_| FullParityFailure::new("FULL_PARITY_STATE_JSON_INVALID"))
}
...
fn string<'a>(payload: &'a Map<String, Value>, key: &str, maximum: usize) -> Result<&'a str> {
    let value = payload.get(key).and_then(Value::as_str).ok_or_else(|| {
        FullParityFailure::new(format!("FULL_PARITY_{}_INVALID", key.to_uppercase()))
    })?;
...
fn integer(payload: &Map<String, Value>, key: &str, minimum: u64, maximum: u64) -> Result<u64> {
    let value = payload.get(key).and_then(Value::as_u64).ok_or_else(|| {
        FullParityFailure::new(format!("FULL_PARITY_{}_INVALID", key.to_uppercase()))
    })?;
...
fn receipt(
    ...
    let mut map = body
        .as_object()
        .cloned()
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_RECEIPT_INVALID"))?;
...
fn envelope(
    ...
    serde_json::to_string(&value)
        .map_err(|_| FullParityFailure::new("FULL_PARITY_JSON_RENDER_FAILED"))
}

fn profile_state(state: &Path) -> Result<Value> {
    let value = read_json(
        &state.join("profiles.json"),
        json!({"profiles": {}, "schema_version": 1, "selected": null}),
    )?;
...
fn profile_result(value: &Value) -> Result<Value> {
    ...
    let profiles = object(
        row.get("profiles")
            .ok_or_else(|| FullParityFailure::new("PROFILE_STATE_INVALID"))?,
    ...
    Ok(
        json!({"profiles": rendered, "selected": row.get("selected").cloned().unwrap_or(Value::Null)}),
    )
}
...
fn profile_apply(root: &Path, project_id: &str, wire: &[u8]) -> Result<Value> {
    let rendered = apply_last_good_json(
        root.to_str()
            .ok_or_else(|| FullParityFailure::new("FULL_PARITY_PROJECT_ROOT_UTF8_INVALID"))?,
        project_id,
        wire,
        None,
        Some(1_800_000_000),
    ...
    serde_json::from_str(&rendered)
        .map_err(|_| FullParityFailure::new("PROFILE_LAST_GOOD_APPLY_INVALID"))
}
...
fn phase_r25(
        ...
        "profile.select" => {
            let wire = {
                ...
                let profile = profiles
                    .get(name)
                    .and_then(Value::as_object)
                    .ok_or_else(|| FullParityFailure::new("PROFILE_NOT_FOUND"))?;
                let wire_hex = profile
                    .get("config_wire_hex")
                    .and_then(Value::as_str)
                    .ok_or_else(|| FullParityFailure::new("PROFILE_STATE_INVALID"))?;
                decode_hex(wire_hex, "PROFILE_CONFIG_WIRE_INVALID")?
            };
            value["selected"] = Value::String(name.to_owned());
            last_good = Some(profile_apply(root, project_id, &wire)?);
        }
    ...
    let last_good_value = if let Some(last_good) = last_good {
        let result = last_good
            .get("result")
            .and_then(Value::as_object)
            .ok_or_else(|| FullParityFailure::new("PROFILE_LAST_GOOD_APPLY_INVALID"))?;
        json!({
            "action": result.get("action").cloned().unwrap_or(Value::Null),
            "config_hash": result.get("config_hash").cloned().unwrap_or(Value::Null),
            "stored_payload_sha256": result
                .get("stored_payload_sha256")
                .cloned()
    ...
    match operation {
        "state.write" => {
            ...
            let content = decode_hex(
                string(payload, "content_hex", MAX_REQUEST_BYTES * 2)?,
                "STATE_WRITE_CONTENT_INVALID",
            )?;
        ...
        "receipt.write" => {
            ...
            let last = std::str::from_utf8(&wire)
                .ok()
...
fn phase_r27(
    ...
    match operation {
        "broker.enqueue" => {
            ...
            if argv.is_empty() || argv.len() > 64 || argv.iter().any(|value| !value.is_string()) {
...
fn value_string_array(value: &str) -> Result<Vec<String>> {
    serde_json::from_str(value)
        .map_err(|_| FullParityFailure::new("INTELLIGENCE_DATABASE_ROW_INVALID"))
}
...
fn memory_search(connection: &Connection, query: &str) -> Result<Value> {
    ...
    for row in rows {
        let (memory_id, text, token_json, tag_json) =
            row.map_err(|_| FullParityFailure::new("MEMORY_QUERY_FAILED"))?;
        let row_tokens = value_string_array(&token_json)?
            .into_iter()
            .collect::<BTreeSet<_>>();
...
fn repository_query(connection: &Connection, query: &str) -> Result<Value> {
    ...
    let mut result = Vec::new();
    for row in rows {
        let (path, content_sha256, token_json, language) =
            row.map_err(|_| FullParityFailure::new("REPOSITORY_QUERY_FAILED"))?;
        let row_tokens = value_string_array(&token_json)?
            .into_iter()
            .collect::<BTreeSet<_>>();
        let score = row_tokens.intersection(&query_tokens).count();
    ...
    let result = match operation {
        "memory.add" => {
            ...
            let tags = payload
                .get("tags")
                .and_then(Value::as_array)
                .ok_or_else(|| FullParityFailure::new("MEMORY_TAGS_INVALID"))?;
        ...
        "memory.search" => {
            json!({"matches": memory_search(&connection, string(payload, "query", 65_536)?)?})
        }
        "repository.index" => {
            let files = payload
                .get("files")
                .and_then(Value::as_object)
                .ok_or_else(|| FullParityFailure::new("REPOSITORY_FILES_INVALID"))?;
            if files.is_empty()
                || files.len() > 2048
                || files.values().any(|value| !value.is_string())
            {
                return failure("REPOSITORY_FILES_INVALID");
            }
            let mut indexed = Vec::with_capacity(files.len());
            for (raw_path, content) in files {
                let path = safe_relative(raw_path)?
                    .to_string_lossy()
                    .replace('\\', "/");
                let content = content
                    .as_str()
                ...
                let token_json = serde_json::to_string(&tokens(content))
                    .map_err(|_| FullParityFailure::new("REPOSITORY_FILES_INVALID"))?;
                connection
                    .execute(
        ...
        "repository.query" => {
            json!({"matches": repository_query(&connection, string(payload, "query", 65_536)?)?})
        }
        _ => return failure("FULL_PARITY_OPERATION_UNSUPPORTED"),
    };
...
fn provider_route(payload: &Map<String, Value>) -> Result<Value> {
    let candidates = payload
        .get("candidates")
        .and_then(Value::as_array)
        .ok_or_else(|| FullParityFailure::new("PROVIDER_ROUTE_INPUT_INVALID"))?;
    let task = payload
        .get("task")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("PROVIDER_ROUTE_INPUT_INVALID"))?;
    ...
    for candidate in candidates {
        let row = candidate
            .as_object()
            .ok_or_else(|| FullParityFailure::new("PROVIDER_CANDIDATE_INVALID"))?;
    ...
    normalized.sort_by(|left, right| {
        left["cost_micros"]
            .as_u64()
            .cmp(&right["cost_micros"].as_u64())
            .then_with(|| {
                left["latency_ms"]
                    .as_u64()
                    .cmp(&right["latency_ms"].as_u64())
            })
    ...
    let selected = normalized
        .first()
        .cloned()
        .ok_or_else(|| FullParityFailure::new("PROVIDER_ROUTE_NO_CANDIDATE"))?;
    ...
    match operation {
        "setup.plan" => {
            let current_hash = if target.exists() {
                Some(digest(&fs::read(&target).map_err(|_| {
                    FullParityFailure::new("SETUP_TARGET_READ_FAILED")
                })?))
            } else {
                None
            };
            let expected_hash = digest(&expected_raw);
            let action = current_hash.as_deref().map_or("create", |value| {
        ...
        "setup.apply" | "setup.repair" => {
            ...
            if target.exists() && !backup.exists() {
                atomic_write(
                    &backup,
                    &fs::read(&target)
                        .map_err(|_| FullParityFailure::new("SETUP_TARGET_READ_FAILED"))?,
                )?;
            }
...
fn benchmark_compare(payload: &Map<String, Value>) -> Result<Value> {
    let baseline = payload
        .get("baseline")
        .and_then(Value::as_array)
        .ok_or_else(|| FullParityFailure::new("BENCHMARK_ARMS_INVALID"))?;
    ...
    for (left, right) in baseline.iter().zip(candidate) {
        let left = left
            .as_object()
            .ok_or_else(|| FullParityFailure::new("BENCHMARK_ROW_INVALID"))?;
        let right = right
            .as_object()
            .ok_or_else(|| FullParityFailure::new("BENCHMARK_ROW_INVALID"))?;
        ...
        ratios.push(
            u64::try_from(ratio).map_err(|_| FullParityFailure::new("BENCHMARK_RATIO_OVERFLOW"))?,
        );
        baseline_quality.push(integer(left, "quality_ppm", 0, 1_000_000)?);
        candidate_quality.push(integer(right, "quality_ppm", 0, 1_000_000)?);
        baseline_success += usize::from(
            left.get("success")
                .and_then(Value::as_bool)
                .unwrap_or(false),
        );
        candidate_success += usize::from(
            right
                .get("success")
                .and_then(Value::as_bool)
                .unwrap_or(false),
...
fn evidence_validate(payload: &Map<String, Value>) -> Result<Value> {
    let receipts = payload
        .get("receipts")
        .and_then(Value::as_array)
        .ok_or_else(|| FullParityFailure::new("EVIDENCE_RECEIPTS_INVALID"))?;
    ...
    for receipt in receipts {
        ...
        if !previous_matches
            || digest(&canonical_bytes(&Value::Object(body.clone()))?) != receipt_hash
        {
...
fn publication_manifest(payload: &Map<String, Value>) -> Result<Value> {
    let artifacts = payload
        .get("artifacts")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("PUBLICATION_ARTIFACTS_INVALID"))?;
    ...
    for (name, row) in artifacts {
        let row = row
            .as_object()
            .ok_or_else(|| FullParityFailure::new("PUBLICATION_ARTIFACT_INVALID"))?;
        safe_relative(name)?;
    ...
    output.insert(
        ...
        Value::String(digest(&canonical_bytes(&body)?)),
    );
    Ok(Value::Object(output))
}
    ...
    match operation {
        "publication.build" => {
            ...
            let manifest_sha = manifest["manifest_sha256"]
                .as_str()
                .ok_or_else(|| FullParityFailure::new("PUBLICATION_MANIFEST_HASH_INVALID"))?;
            write_json(
        ...
        "publication.verify" => {
            ...
            let map = value
                .as_object()
                .ok_or_else(|| FullParityFailure::new("PUBLICATION_MANIFEST_NOT_FOUND"))?;
            let mut body = map.clone();
            let stored = body
                .remove("manifest_sha256")
                .and_then(|value| value.as_str().map(str::to_owned));
            let valid = stored.as_deref() == Some(manifest_sha)
...
fn distribution_manifest(payload: &Map<String, Value>) -> Result<Value> {
    ...
    let files = payload
        .get("files")
        .and_then(Value::as_array)
        .ok_or_else(|| FullParityFailure::new("DISTRIBUTION_INPUT_INVALID"))?;
    ...
    match operation {
        ...
        "distribution.verify" => {
            let manifest = payload
                .get("manifest")
                .and_then(Value::as_object)
                .ok_or_else(|| FullParityFailure::new("DISTRIBUTION_MANIFEST_INVALID"))?;
            let mut body = manifest.clone();
            let digest_value = body
                .remove("distribution_sha256")
                .and_then(|value| value.as_str().map(str::to_owned));
            let forbidden = body.remove("forbidden_python_files");
            let actual_digest = digest(&canonical_bytes(&Value::Object(body.clone()))?);
            let valid = digest_value.as_deref() == Some(actual_digest.as_str())
...
fn phase_r37(operation: &str, payload: &Map<String, Value>) -> Result<(Value, Mutation)> {
    ...
    let phases = payload
        .get("phases")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("CERTIFICATION_INPUT_INVALID"))?;
    let dimensions = payload
        .get("dimensions")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("CERTIFICATION_INPUT_INVALID"))?;
    let phase_complete = (25..=36)
        .all(|phase| phases.get(&format!("R{phase}")).and_then(Value::as_bool) == Some(true));
    let dimension_complete = [
        "cli",
        "host_setup",
        "mcp",
        "platform_packaging",
...
fn mcp_call(payload: &Map<String, Value>, root: &Path, project_id: &str) -> Result<Value> {
    ...
    let arguments = payload
        .get("arguments")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("MCP_ARGUMENTS_INVALID"))?;
    match tool {
        "syntavra.parity.status" => Ok(
            json!({"claim": "FULL_PARITY_PROVEN", "phases": (25..=37).map(|value| format!("R{value}")).collect::<Vec<_>>(), "product_version": "0.0.1"}),
        ),
        "syntavra.context.rewrite" => rewrite_text(arguments),
        "syntavra.provider.route" => provider_route(arguments),
        "syntavra.memory.search" => {
            phase_r30("memory.search", arguments, root).map(|value| value.0)
        }
        "syntavra.repository.query" => {
            phase_r30("repository.query", arguments, root).map(|value| value.0)
        }
        "syntavra.benchmark.compare" => benchmark_compare(arguments),
        "syntavra.profile.list" => {
            phase_r25("profile.list", arguments, root, project_id).map(|value| value.0)
        }
        "syntavra.broker.list" => phase_r27("broker.list", arguments, root).map(|value| value.0),
        "syntavra.setup.verify" => {
            phase_r33("setup.verify", arguments, root, project_id).map(|value| value.0)
        }
        "syntavra.publication.verify" => {
            phase_r35("publication.verify", arguments, root).map(|value| value.0)
        }
    ...
    match operation {
        ...
        "mcp.call" => {
            ...
            let database = matches!(
                tool,
                "syntavra.memory.search" | "syntavra.repository.query" | "syntavra.broker.list"
            );
            Ok((
...
pub fn execute_json(
    ...
    let request_map = request_value
        .as_object()
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_REQUEST_INVALID"))?;
    if request_map.get("schema_version").and_then(Value::as_u64) != Some(SCHEMA_VERSION) {
        return failure("FULL_PARITY_SCHEMA_UNSUPPORTED");
    }
    let phase = request_map
        .get("phase")
        .and_then(Value::as_str)
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_ROUTE_INVALID"))?;
    let operation = request_map
        .get("operation")
        .and_then(Value::as_str)
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_ROUTE_INVALID"))?;
    let payload = request_map
        .get("payload")
        .and_then(Value::as_object)
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_PAYLOAD_INVALID"))?;
    let phase_number = phase
        .strip_prefix('R')
        .and_then(|value| value.parse::<u8>().ok())
        .filter(|value| (25..=37).contains(value))
        .ok_or_else(|| FullParityFailure::new("FULL_PARITY_ROUTE_INVALID"))?;
...
pub fn child_mode(mode: &str, value_hex: &str) -> std::result::Result<i32, String> {
    ...
    match mode {
        ...
        "sleep" => {
            let seconds = value
                .parse::<f64>()
                .map_err(|_| "PROCESS_CHILD_VALUE_INVALID".to_owned())?;
