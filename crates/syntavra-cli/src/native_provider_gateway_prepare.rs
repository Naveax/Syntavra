...
fn capabilities(provider: &str) -> Result<ProviderCapabilities, String> {
    match provider.trim().to_lowercase().as_str() {
        ...
        | "openai-compatible" => Ok(ProviderCapabilities {
...
fn atomic_write(path: &Path, payload: &[u8]) -> Result<(), String> {
    ...
    fs::create_dir_all(parent)
        .map_err(|error| format!("PROVIDER_ATOMIC_CREATE_FAILED:{error}"))?;
...
fn cache_plan(
    ...
    save_cache_plan(
        state_root,
        &format!("{provider}:{model}:{}", plan["stable_prefix_hash"].as_str().unwrap_or_default()),
        plan,
    )?;
    Ok((
        CachePlanInfo {
...
fn temperature(request: &Value) -> f64 {
    ...
    let raw = map
        .get("temperature")
        .or_else(|| map.get("generationConfig").and_then(Value::as_object).and_then(|row| row.get("temperature")))
        .or_else(|| map.get("generation_config").and_then(Value::as_object).and_then(|row| row.get("temperature")));
    match raw {
...
fn replay_lookup(state_root: &Path, cache_key: &str) -> Result<String, String> {
    ...
    if handle.is_none() {
        transaction
            .execute("DELETE FROM provider_response_cache WHERE expires_at<=?1", [now])
...
pub(crate) fn prepare(state_root: &Path) -> Result<Value, String> {
    ...
    reject_credentials(&request, "request")?;

    let mut prepared = request.clone();
    let resolved_model = if !model_option.is_empty() {
        model_option
    } else {
        py_string(
            prepared.as_object().and_then(|map| map.get("model")),
            "unknown",
        )
    };
