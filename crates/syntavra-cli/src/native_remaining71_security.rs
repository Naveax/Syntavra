...
pub(crate) fn supports(command: &[String]) -> bool {
    command.len() == 2
        && command[0] == "run"
        && matches!(command[1].as_str(), "capability-decide" | "capability-issue" | "capability-verify")
}
...
fn now_iso() -> Result<String, String> {
    let duration = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|error| format!("CAPABILITY_CLOCK_FAILED:{error}"))?;
    let seconds = i64::try_from(duration.as_secs()).map_err(|_| "CAPABILITY_CLOCK_RANGE".to_owned())?;
...
fn repeated_values(arguments: &[String], flag: &str) -> Result<Vec<String>, String> {
    ...
    while index < arguments.len() {
        if arguments[index] == flag {
            index += 1;
            output.push(arguments.get(index).ok_or_else(|| format!("{flag}_VALUE_MISSING"))?.clone());
...
fn load_json(value: &str) -> Result<Value, String> {
    let raw = if Path::new(value).is_file() {
        fs::read_to_string(value).map_err(|error| format!("CAPABILITY_ARGUMENTS_READ_FAILED:{error}"))?
    } else {
        value.to_owned()
    };
...
fn category(tool: &str) -> &'static str {
    ...
    for prefix in ["read", "search", "list", "find", "inspect", "grep"] {
        if leaf.starts_with(prefix) { return "read"; }
    }
    for prefix in ["write", "edit", "patch", "delete", "move", "create", "update"] {
        if leaf.starts_with(prefix) { return "write"; }
    }
    for prefix in ["run", "exec", "shell", "terminal", "bash", "powershell", "cmd"] {
        if leaf.starts_with(prefix) { return "execute"; }
    }
    for prefix in ["web", "http", "fetch", "download", "upload", "request", "browser"] {
        if leaf.starts_with(prefix) { return "network"; }
    }
    "unknown"
}
...
fn decide(tool: &str, arguments: &Value, resource: &str, sandboxed: bool, user_authorized: bool, network_allowlist: &[String]) -> Result<Value, String> {
    ...
    let policy_text = arguments
        ...
        .map(|rows| rows.iter().map(|value| value.as_str().map(str::to_owned).unwrap_or_else(|| value.to_string())).collect::<Vec<_>>().join(" "))
    ...
    if matches!(category, "write" | "execute" | "network") {
        ...
        if !user_authorized { allowed = false; reason = "authorization-required"; }
    }
    if category == "execute" {
    ...
    if category == "network" && !network_allowlist.is_empty() {
        let host = arguments.as_object().and_then(|map| map.get("host")).and_then(Value::as_str).unwrap_or_default();
...
impl CapabilityStore {
    fn open(state_root: &Path) -> Result<Self, String> {
        fs::create_dir_all(state_root).map_err(|error| format!("CAPABILITY_STATE_CREATE_FAILED:{error}"))?;
        ...
        if !key_path.exists() && legacy_key.exists() {
            fs::rename(&legacy_key, &key_path).map_err(|error| format!("CAPABILITY_LEGACY_KEY_MOVE_FAILED:{error}"))?;
        }
        if !key_path.exists() {
            ...
            output.write_all(&key).map_err(|error| format!("CAPABILITY_KEY_WRITE_FAILED:{error}"))?;
            output.flush().map_err(|error| format!("CAPABILITY_KEY_FLUSH_FAILED:{error}"))?;
            #[cfg(unix)]
            {
            ...
            fs::rename(&temporary, &key_path).map_err(|error| format!("CAPABILITY_KEY_RENAME_FAILED:{error}"))?;
        }
        let key = fs::read(&key_path).map_err(|error| format!("CAPABILITY_KEY_READ_FAILED:{error}"))?;
        ...
        if !db_path.exists() && legacy_db.exists() {
            fs::rename(&legacy_db, &db_path).map_err(|error| format!("CAPABILITY_LEGACY_DB_MOVE_FAILED:{error}"))?;
            for suffix in ["-wal", "-shm"] {
                let legacy = Path::new(&format!("{}{}", legacy_db.to_string_lossy(), suffix)).to_path_buf();
                if legacy.exists() {
                    let target = Path::new(&format!("{}{}", db_path.to_string_lossy(), suffix)).to_path_buf();
                    let _ = fs::rename(legacy, target);
                }
            }
        }
        let db = Connection::open(db_path).map_err(|error| format!("CAPABILITY_DB_OPEN_FAILED:{error}"))?;
    ...
    fn issue(&self, session_id: &str, tool: &str, arguments: &Value, resource: &str, permissions: &[String], ttl: i64, single_use: bool) -> Result<String, String> {
        let now = unix_seconds()?;
        let mut nonce_raw = [0u8; 18];
        OsRng.fill_bytes(&mut nonce_raw);
        let permissions = permissions.iter().cloned().collect::<BTreeSet<_>>().into_iter().collect::<Vec<_>>();
        let body = json!({
            "version": VERSION,
            "channel": CHANNEL,
    ...
    fn verify(&self, token: &str, tool: &str, arguments: &Value, resource: &str, consume: bool) -> Result<Value, String> {
        ...
        let used: Option<i64> = self.db
            .query_row("SELECT 1 FROM consumed WHERE nonce=?1", [nonce], |row| row.get(0))
            .optional()
        ...
        if consume && body["single_use"].as_bool().unwrap_or(true) {
            self.db.execute("INSERT INTO consumed VALUES(?1,?2)", params![nonce, now_iso()?])
...
pub(crate) fn execute(command: &[String], arguments: &[String], state_root: &Path) -> Result<Option<Value>, String> {
    ...
    let value = match action {
        "capability-decide" => {
            if positional.len() < 2 { return Err("CAPABILITY_DECIDE_ARGUMENTS_REQUIRED".to_owned()); }
            let args = load_json(&positional[1])?;
            decide(
                &positional[0],
                &args,
                option_value(arguments, "--resource")?.as_deref().unwrap_or("workspace:/"),
        ...
        "capability-issue" => {
            if positional.len() < 3 { return Err("CAPABILITY_ISSUE_ARGUMENTS_REQUIRED".to_owned()); }
            let args = load_json(&positional[2])?;
            let store = CapabilityStore::open(&security_root)?;
            let ttl = option_value(arguments, "--ttl")?
                .map(|value| value.parse::<i64>().map_err(|_| "CAPABILITY_TTL_INVALID".to_owned()))
                .transpose()?.unwrap_or(300);
            let single_use = !has_flag(arguments, "--reusable");
            let token = store.issue(
                &positional[0], &positional[1], &args,
                option_value(arguments, "--resource")?.as_deref().unwrap_or("workspace:/"),
                &repeated_values(arguments, "--permission")?, ttl, single_use,
            )?;
            json!({"ok": true, "token": token, "single_use": single_use})
        }
        "capability-verify" => {
            if positional.len() < 3 { return Err("CAPABILITY_VERIFY_ARGUMENTS_REQUIRED".to_owned()); }
            let args = load_json(&positional[2])?;
            let store = CapabilityStore::open(&security_root)?;
            store.verify(
                &positional[0], &positional[1], &args,
