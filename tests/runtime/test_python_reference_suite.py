...
class PythonReferenceSuiteArchitectureTests(unittest.TestCase):
    ...
    def test_family_report_validation_rejects_unexpected_nondeterminism(self) -> None:
        good = {
            "ok": True,
    ...
    def test_isolated_environment_removes_live_credentials_and_preserves_localhost(self) -> None:
        with tempfile.TemporaryDirectory(prefix="syntavra-reference-suite-env-") as directory:
            ...
            seeded.update({"PATH": os.environ.get("PATH", ""), "PYTHONPATH": "old"})
        ...
        self.assertTrue(env["HOME"].startswith(str(scratch)))
    ...
    def test_suite_contract_is_fail_closed(self) -> None:
        ...
        self.assertTrue(execution["isolated_home"])
