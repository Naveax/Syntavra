...
class ProcessBrokerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.broker = ProcessBroker(
            self.root / "broker",
            EvidenceStore(self.root / "evidence", project_id="p"),
    ...
    def test_synchronous_success_and_failure(self):
        success = self.broker.run(
            (sys.executable, "-c", "print('ok')"),
            cwd=self.root,
            timeout=5,
        )
        self.assertEqual(success.exit_code, 0)
        self.assertTrue(success.evidence_handle)

        failure = self.broker.run(
            (
                sys.executable,
                "-c",
                "import sys; print('ERROR root'); sys.exit(3)",
            ),
            cwd=self.root,
            timeout=5,
        )
        self.assertEqual(failure.exit_code, 3)
        self.assertIn("ERROR root", failure.summary)
    ...
    def test_background_job_completes_without_broker_poll_loop(self):
        # Detached process startup on shared Windows CI runners can exceed five
        # seconds under load. The test verifies durable background completion,
        # not an aggressive process-start latency target.
        ...
        self.assertEqual(current.state, "COMPLETED", repr(current))
        self.assertTrue(self.broker.completions.is_file())
