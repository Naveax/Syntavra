...
class RolloutContextTests(unittest.TestCase):
    ...
    def test_incremental_partial_and_duplicate_events(self):
        ...
        second=RolloutTailer(rollout,state).poll(); self.assertEqual(second["processed_events"],1); self.assertEqual(second["counters"]["model_turns"],1); self.assertEqual(second["counters"]["duplicate_events"],1); self.assertEqual(second["counters"]["fresh_input_tokens"],10)
