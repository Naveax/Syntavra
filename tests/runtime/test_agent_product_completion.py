...
class AgentProductCompletionTests(unittest.TestCase):
    def test_structured_edit_compiler_is_exact_and_git_apply_compatible(self) -> None:
    ...
    def test_repository_query_connection_is_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_gateway_structured_edit_and_event_stream(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            project = Path(temporary) / "project"
            project.mkdir()
            ...
            self.assertEqual([item["action"] for item in provider.trace], ["search", "inspect", "edit"])
            self.assertEqual([event.sequence for event in events], list(range(1, len(events) + 1)))
            self.assertTrue(any(event.event_type == "patch-proposed" for event in events))

    def test_delivery_apply_requires_authorization_and_uses_verified_diff(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            project = Path(temporary) / "project"
            project.mkdir()
            subprocess.run(["git", "init"], cwd=project, check=True, capture_output=True)
